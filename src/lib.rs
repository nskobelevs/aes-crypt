...
macro_rules! create_aes_functions {
    ( $key_length:literal, $key_words_count:literal, $round_count:literal) => {
        paste! {
            #[doc = concat!("Expands a ", stringify!($key_length), " bit length key for AES-", stringify!($key_length), " encryption.")]
            pub const fn [<expand_ $key_length _bit_key>](key: [u32; $key_words_count]) -> [[u32; 4]; $round_count] {
                let mut key_words = [0u32; $round_count * 4];
                ...
                let mut round_keys = [[0u32; 4]; $round_count];

                let mut i = 0;

                while i < key_words.len() {
                    round_keys[i / 4][i % 4] = key_words[i];
                    i += 1;
                }
...
create_aes_functions!(128, 4, 11);
create_aes_functions!(196, 6, 13);
create_aes_functions!(256, 8, 15);

/// Performs XOR on each byte of a state with each byte of a round key
#[allow(non_snake_case)]
pub fn AddRoundKey(state: &mut [u32; 4], round_key: [u32; 4]) {
...
mod tests {
    ...
    pub fn test_add_round_key() {
        ...
        AddRoundKey(&mut input, expanded_key[0]);
